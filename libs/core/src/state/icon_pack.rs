    ...
    pub fn matches(&self, entry: &IconPackEntry) -> bool {
        match (self, entry) {
            (IconPackEntry::Unique(self_unique), IconPackEntry::Unique(other_unique)) => {
                let umid_matches = self_unique.umid == other_unique.umid;
                let path_matches = match (&self_unique.path, &other_unique.path) {
                    (Some(p1), Some(p2)) => {
                ...
                let redirect_matches = match (&self_unique.redirect, &other_unique.redirect) {
                    (Some(r1), Some(r2)) => {
                        r1.to_string_lossy().to_lowercase() == r2.to_string_lossy().to_lowercase()
                    }
                    (None, None) => true,
                    _ => false,
                };
                umid_matches && path_matches && redirect_matches
            }
            (IconPackEntry::Shared(self_shared), IconPackEntry::Shared(other_shared)) => {
                self_shared.extension.to_lowercase() == other_shared.extension.to_lowercase()
            }

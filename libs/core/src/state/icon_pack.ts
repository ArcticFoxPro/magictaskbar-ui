...
export class IconPackList extends List<IIconPack> {
  static getAsync(): Promise<IconPackList> {
    return newFromInvoke(this, FuncCommand.StateGetIconPacks);
  }

  static onChange(cb: (payload: IconPackList) => void): Promise<UnSubscriber> {
    return newOnEvent(cb, this, FuncEvent.StateIconPacksChanged);
  }
}
...
export class IconPackManager {
   ...
   * const iconPath = instance.getIconPath({
   *   umid: "MagicTaskbar.UI_p6yyn03m1894e!App"
   * });
   */
  public getIconPath(args: FuncCommandGetIconArgs): IIcon | null {
    ...
    for (const pack of this.activeIconPacks) {
      ...
      if (umid) {
        entry = pack.entries.find((e) =>
          e.type === "unique" && !!e.umid && e.umid === umid
      ...
      if (!entry && lowerPath) {
        entry = pack.entries.find((e) => {
          ...
          // 完整路径精确匹配
          if (e.path === lowerPath) {
            console.log(`[IconPack] ✓ 完整路径匹配成功：${lowerPath}`);
            return true;
          }

          // 文件名匹配（备选）
          // 但排除通用进程名（browser.exe, launcher.exe 等），避免缓存冲突
          if (extension === "exe") {
            const filename = lowerPath.split("\\").pop();
            // 通用进程名列表
            const genericExenames = [
              "browser.exe",
              "launcher.exe",
              "setup.exe",
              "install.exe",
            ...
            const isGenericExe = filename && genericExenames.includes(filename);

            if (isGenericExe) {
              // 通用进程名：必须用完整路径匹配，不能用文件名匹配
              return false;
            }

            // 🔧 使用精确的文件名匹配，避免部分匹配导致图标冲突
            // 例如：DeltaForceClient-Win64-Shipping.exe 会被 Client-Win64-Shipping.exe 错误匹配
            const cachedFilename = e.path.split("\\").pop();
            if (filename && cachedFilename && filename === cachedFilename) {
              console.log(
                `[IconPack] ✓ 文件名匹配：请求=${lowerPath}, 缓存=${e.path}, 文件名=${filename}`,
              );
              return true;
            }
          }
      ...
      if (!entry) {
        console.log(`[IconPack] ✗ 未找到图标：${lowerPath}`);
      }
      ...
      if (entry) {
        if (entry.redirect) {
          ...
          return this.getIconPath(
            { path: entry.redirect, __seen } as FuncCommandGetIconArgs,
          );
        }
  ...
  public getIcon({ path, umid }: FuncCommandGetIconArgs): IIcon | null {
  ...
  public static requestIconExtraction(
    obj: FuncCommandGetIconArgs,
  ): Promise<void> {
    return invoke(FuncCommand.GetIcon, obj);
  }
  ...
  public static clearCachedIcons(): Promise<void> {
    return invoke(FuncCommand.StateDeleteCachedIcons);
  }
}

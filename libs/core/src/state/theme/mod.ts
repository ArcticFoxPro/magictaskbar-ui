...
export class ThemeList extends List<ITheme> {
  static getAsync(): Promise<ThemeList> {
    return newFromInvoke(this, FuncCommand.StateGetThemes);
  }

  static onChange(cb: (payload: ThemeList) => void): Promise<UnSubscriber> {
    return newOnEvent(cb, this, FuncEvent.StateThemesChanged);
  }
  ...
  applyToDocument(activeIds: ThemeId[], variables: ISettings["byTheme"]): void {
    ...
    // sort by user order
    enabledThemes.sort((a, b) =>
      activeIds.indexOf(a.id) - activeIds.indexOf(b.id)
    );
...
export class Theme {
  ...
  applyToDocument(varValues: ISettings["byTheme"][ResourceId] = {}): void {
    // Get widget ID from window label (format: @effect/widget-name?params)
    const widgetId =
      window.__TAURI_INTERNALS__?.metadata?.label?.split("?")[0] ||
      "@effect/taskbar";
    let styles = ``;

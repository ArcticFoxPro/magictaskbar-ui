...
import type { Color as IColor, UIColors as IUIColors } from "@magic-ui/types";

export class UIColors {
  constructor(public inner: IUIColors) {}

  static getAsync(): Promise<UIColors> {
    return newFromInvoke(this, FuncCommand.SystemGetColors);
  }

  static onChange(cb: (payload: UIColors) => void): Promise<UnSubscriber> {
    return newOnEvent(cb, this, FuncEvent.ColorsChanged);
  }
...
export class Color {
  ...
  setAsCssVariable(name: string): void {
    ...
    this.insertIntoStyleSheet({
      ...
      [`--color-${parsedName}-rgb`]:
        `${this.inner.r}, ${this.inner.g}, ${this.inner.b}`,
      [`--color-${parsedName}-rgba`]:
        `${this.inner.r}, ${this.inner.g}, ${this.inner.b}, ${this.inner.a}`,
    });
  }

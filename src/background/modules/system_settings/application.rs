...
use libs_core::system_state::UIColors;
...
use super::infrastructure::CURRENT_IS_DARK;
...
impl SystemSettings {
    ...
    fn internal_on_colors_change(
        ...
        // 直接新建 UISettings 实例读取 background 颜色更新全局状态
        // 不锁 SYSTEM_SETTINGS，避免锁竞争
        if let Ok(ui) = UISettings::new() {
            if let Ok(bg) = ui.GetColorValue(UIColorType::Background) {
                let is_dark = bg.R < 128;
                CURRENT_IS_DARK.store(is_dark, Ordering::SeqCst);
                log::info!(
                    "[SystemSettings] Theme changed: is_dark={} (bg.R={})",
                    is_dark,
                    bg.R
                );
            }
